...
impl PartialOrd for IntegerRotation {
    fn partial_cmp( &self, other: &IntegerRotation ) -> Option<Ordering> {
        Some(u64::cmp(&self.n, &other.n)
            .then(i64::cmp(&(other.b * self.a), &(self.b * other.a))))
    }
}
...
###.##.####.##.#..##");
        ...
        assert_eq!(nth_target(&m, max_visibility(&m).0, 1), &Location{ x: 11, y: 12 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 2), &Location{ x: 12, y: 1 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 3), &Location{ x: 12, y: 2 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 10), &Location{ x: 12, y: 8 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 20), &Location{ x: 16, y: 0 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 50), &Location{ x: 16, y: 9 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 100), &Location{ x: 10, y: 16 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 199), &Location{ x: 9, y: 6 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 200), &Location{ x: 8, y: 2 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 201), &Location{ x: 10, y: 9 });
        assert_eq!(nth_target(&m, max_visibility(&m).0, 299), &Location{ x: 11, y: 1 });
    }
}
